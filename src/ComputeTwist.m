...
/**************************************************
Given generators P,Q for Kum(J)[pp] over
F_q chooses a symplectic basis in their span. Idea
...
/**************************************************
Given an qq-adic approximation for a Hilbert 90 
...
function KQT(f, g, h : bigger_p:=0)
    ...
    possible_subgroups := NormalSubgroups(GG : IndexEqual:=6);

    maybe_error_msg := "The computed Galois group cannot possibly be GL_2(F_7)/{\pm 1}.\n";
    maybe_error_msg := maybe_error_msg cat "  probably we did not have enough precision when computing the degree\n";
    maybe_error_msg := maybe_error_msg cat "  24 polynomials, sorry.";
    error if #possible_subgroups eq 0, maybe_error_msg;
    HH := possible_subgroups[1]`subgroup; // K^HH = QQ(zeta)
    ...
    vprint TalkToMe, 1 : "Working mod q to choose a symplectic basis for K[pp]";
    ...
    pts_set := &join{&join{{pt : pt in Points(Kum_p, [1,a,b]) | 7*pt eq Kum_p!0} : b in rts2_p} : a in rts_p};
    pts_set := {pt : pt in pts_set | pt ne Kum_p!0};
    pts_set := {pt : pt in pts_set | (2*pt in pts_set) and (3*pt in pts_set)}; //protect against freak stuff
    
    maybe_error_msg := "The number of 7-torsion points we found, in the correct\n";
    maybe_error_msg := maybe_error_msg cat Sprintf("  place, was %o (not 24). ", #pts_set);
    maybe_error_msg := maybe_error_msg cat "I don't understand this error at all.";
    error if #pts_set ne 24, maybe_error_msg;
    ...
    vprint TalkToMe, 1 : "Computing qq-adic approximation to the cocycle";
...
intrinsic KleinQuarticTwists(f::RngUPolElt, pm::RngIntElt : max_tries:=max_tries) -> SeqEnum 
...
antisymplectically) isomorphic to Jac(C)[pp].}
    QQ := Rationals();
    _<x> := PolynomialRing(QQ);
    ...
    vprint TalkToMe, 1 : "NB: An error here is most likely because the coefficients are";
    vprint TalkToMe, 1 : "    too big for the max precision to deal with.";
    g,h := Poly24(f : pm:=pm);
    vprint TalkToMe, 1 : "Done computing polynomials\n";
    ...
    while (not done) and (counter lt max_tries) do
        try
            tw1 := KQT(Evaluate(f,x), g, h : bigger_p:=counter);
            vprint TalkToMe, 1 : Sprint([PP!DefiningPolynomial(c) : c in tw1]) cat "\n";
            done := true;
        catch e
            e;
            counter+:=1;
            if counter ne max_tries then
                vprintf TalkToMe, 1: "ERROR: Fail on attempt %o, try again with new prime.\n", counter;
            else
                vprint TalkToMe, 1: "ERROR: Failed out on final attempt, exit.\n";
            end if;
        end try;
    end while;
    return tw1;
...
intrinsic KleinQuarticTwists(f::RngUPolElt : max_tries:=4) -> SeqEnum, MonStgElt
{Given a genus 2 curve C : y^2 = f(x) with RM by O_D compute the twists of 
X(7) which parametrise elliptic curves whose 7-torsion is (symplectically or 
antisymplectically) isomorphic to Jac(C)[pp].}
    error_str := "";
    ret := [];
    XX := KleinQuarticTwists(f, 1 : max_tries:=max_tries);
    if #XX eq 2 then
        ret := ret cat XX;
    else
        error_str := error_str cat "Ran into an error for sign +1.";
    end if;
...
intrinsic KleinQuarticTwists(C::CrvHyp : max_tries:=4) -> SeqEnum, MonStgElt
{Given a genus 2 curve C : y^2 = f(x) with RM by O_D compute the twists of 
X(7) which parametrise elliptic curves whose 7-torsion is (symplectically or 
antisymplectically) isomorphic to Jac(C)[pp].}
    _<x> := PolynomialRing(Rationals());
    f := Evaluate(-DefiningPolynomial(C), [x,0,1]);
    return KleinQuarticTwists(f : max_tries:=max_tries);
end intrinsic;


intrinsic KleinQuarticTwists(APQ::SeqEnum : max_tries:=4) -> SeqEnum
{Given a triple of Bending's parameters [A,P,Q] compute the twists of
X(7) which parametrise elliptic curves whose 7-torsion is (symplectically or
antisymplectically) isomorphic to Jac(C)[pp] using the numerically stable
approach.}
  QQ := Rationals();
  _<x> := PolynomialRing(QQ);
  f := Genus2Curve(APQ);
